package utilidades;
//Esta es la forma indicada para leer desde consola a partir de la versiÃ³n Java 1.5
...
public class teclado {
	...
	public static int leer_entero (String mensaje, int min, int max) {
		...
		do {
			...
			if (n < min || n > max) { //Se cumple o no se cumple la condición de que n sea mayor o menor que...?
				System.out.println("Debe introducir un número entre "+ min + " y " + max);
			}else {
				return n;
			}
		} while (true); // Se cumple la condición?
	}
	public static int leer_entero (String mensaje) {
		...
		// mostramos mensaje de error en caso de lectura incorrecta (p.e leer una letra en vez de un nÂº) 
		catch (Exception e){
			System.err.println ("Error de lectura del nÃºmero. CÃ³digo de error = " + e);
	    e.printStackTrace();
			System.exit(1);
		}			
	...
	/* in: recibe una cadena
		 out: devuelve un double leÃ­do desde teclado, muestra por pantalla mensaje
		*/
	public static double leer_double (String mensaje) {			
			Scanner entrada = new Scanner (System.in);			

			// leer un real de doble precisiÃ³n desde la consola 
			double real1=0;
			System.out.print (mensaje);	
			...
			// devolver el real leÃ­do desde teclado
			return real1;
	} // fin leer_double
	...
	public static void main (String args[]) {	
		...
		int numero = leer_entero ("Introduce un nÃºmero: ");
		System.out.println ("El nÃºmero leido es: " + numero);			
